...
void ShapesDialog::setConfiguration(const char * conf) 
{
    ...
    if(!conf)
    {
        ...
        zenoh::Config config;
        zenoh::Session s = std::get<zenoh::Session>(zenoh::open(std::move(config)));
        session = std::make_shared<zenoh::Session>(std::move(s));
    }
    else{

        std::cout<<"from json5 file configuration. file Name: "<<conf<<std::endl;
        zenoh::Config config = std::get<zenoh::Config>(zenoh::config_from_file(conf));
         auto s = std::get<zenoh::Session>(zenoh::open(std::move(config)));
         session = std::make_shared<zenoh::Session>(std::move(s));
...
ShapesDialog::onPublishButtonClicked()
{
    ...
    ShapeType shape;
    shape.color((char*)colorString_[cIdx]);
    ...
    switch (sIdx)
    {
        ...
        case CIRCLE:
        {
            ...
            const std::string& keyExpression = "Circle";
            std::cout<<"keyExpression: "<<keyExpression<<std::endl;
            zenoh::KeyExprView keyExp(keyExpression);
            zenoh::Publisher p = std::get<zenoh::Publisher>(session->declare_publisher(keyExp));
        ...
        case SQUARE:
        {
           ...
           //Publisher p = std::get<Publisher>(session->declare_publisher("demo/ishape/shapeType/square"));
            ...
            const std::string& keyExpression = "Square";
            zenoh::KeyExprView keyExp(keyExpression);
            zenoh::Publisher p = std::get<zenoh::Publisher>(session->declare_publisher(keyExp));
        ...
        case TRIANGLE:
        {
            ...
            const std::string& keyExpression = "Triangle";
            zenoh::KeyExprView keyExp(keyExpression);
    ...
    switch (sIdx)
    {
        ...
        case SQUARE:
        {
            ...
            for (int i = 0; i < 8; ++i)
            {
                    ...
                    const std::string& keyExp = "Square";
                    ZenohShapeDynamics::ref_type
                    dynamics(new ZenohShapeDynamics(x, y, session, keyExp, colorStr, i, true));
        ...
        case TRIANGLE:
        {
           ...
           // std::shared_ptr<zenoh::Subscriber> ssubscriber_;
            for (int i = 0; i < 8; ++i)
            {
                    ...
                    const std::string& keyExp = "Triangle";
